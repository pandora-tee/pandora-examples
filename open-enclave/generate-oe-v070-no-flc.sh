...
OUTPUT_DIR=./bin-and-sgxtrace-v070
...
docker build --target sdk -t oe-v0.7.0 --network=host  -f ./open-enclave-v070.Dockerfile .
...
docker run -v $OUTPUT_DIR:/root -w /root -v ${SGX_TRACE_RESOLVED}:/sgx-tracer --cap-add=SYS_PTRACE --device /dev/isgx -v /var/run/aesmd:/var/run/aesmd -i -t oe-v0.7.0 bash -c "rm -rf /root/* && cp -r /openenclave/samples/helloworld /root && /sgx-tracer/sgx-tracer /root/helloworld/host/helloworldhost /root/helloworld/enclave/helloworldenc.signed > /root/ptrace.log"; 
 
echo "------------------------------------------------"
echo "Done!"
echo "------------------------------------------------"
 
cp $OUTPUT_DIR/helloworld/enclave/helloworldenc $OUTPUT_DIR/enclave0.so 
ls -la $OUTPUT_DIR
